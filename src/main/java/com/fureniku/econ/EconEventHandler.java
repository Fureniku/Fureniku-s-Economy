...
public class EconEventHandler {
	...
	public void onPlayerLogout(PlayerLoggedOutEvent event) {
		...
		if (cstmr.getCartPos() != null && cstmr.getCartPos().getY() > 0) {
			...
			if (customer != null) {
				StoreManagerEntity manager = customer.getCart(player.world).getManager();
				player.sendMessage(new TextComponentTranslation("econ.event.player_log_out_cart", manager.getShopName()));
				manager.removeActiveCustomer(customer, 4);
			}
		}
	}
	
	@SubscribeEvent
	public void unloadShopManager(ChunkDataEvent.Unload event) {

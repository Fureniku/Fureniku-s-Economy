...
import com.fureniku.econ.network.ATMWithdrawPacket;
import com.fureniku.econ.network.SoundPacket;
...
public class GuiATM extends GuiContainer {
	...
	protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
    	...
    	String shortAmt = EconUtils.formatBalance(withdrawAmount - initBalance);
    	String underScore = "";
    	if (tick < 80) {
    		tick++;
    	} else if (tick >= 80) {
    	...
    	if (guiStage == 5) {
    		...
    		drawCenteredString(fontRenderer, EconUtils.formatBalance(withdrawAmount*100), centre, y + 52, fontColour);

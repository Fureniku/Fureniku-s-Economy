...
public class AdminShopPricePacket implements IMessage {
	...
	public static class Handler implements IMessageHandler<AdminShopPricePacket, IMessage> {
		...
		public IMessage onMessage(AdminShopPricePacket message, MessageContext ctx) {
			FurenikusEconomy.log(3, "[Network] " + String.format("Packet recieved! 9 bits of data? %s %s %s %s %s etc", AdminShopPricePacket.ownerName, AdminShopPricePacket.buyPrice1, AdminShopPricePacket.sellPrice1, AdminShopPricePacket.buyPrice2, AdminShopPricePacket.sellPrice2));
			return null;
		}
	}
}